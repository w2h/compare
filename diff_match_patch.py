...
class diff_match_patch:
  ...
  def diff_compute(self, text1, text2, checklines, deadline):
    ...
    i = diff_match_patch.find(longtext, shorttext)
    if i != -1:
  ...
  def diff_lineMode(self, text1, text2, deadline):
    ...
    # (text1, text2, linearray) = self.diff_linesToChars(text1, text2) # howie disable this line
    ...
    # self.diff_cleanupSemantic(diffs)# howie disable this line
    ...
    count_delete = 0
    count_insert = 0
    text_delete = [] # howie modify str to list
    text_insert = [] # howie modify str to list
    while pointer < len(diffs):
      if diffs[pointer][0] == self.DIFF_INSERT:
        count_insert += 1
        text_insert.extend(diffs[pointer][1]) # howie modify str to list
  ...
  def diff_halfMatch(self, text1, text2):
    ...
    def diff_halfMatchI(longtext, shorttext, i):
      ...
      best_common = ''
      j = diff_match_patch.find(shorttext,seed)
      while j != -1:
  ...
  def get_lcs(self,diffs,isArr=True):
  ...
  def get_lcs_indexs_by_text1(self,diffs,text1):
      result = []
      last_index = -1
      for (op, data) in diffs:
          if op == self.DIFF_EQUAL:
              startIndex = self.find(text1, data[0], last_index + 1)
              result.append(startIndex)
              le = len(data)
              if le > 1:
                  for i in range(1, le):
                      result.append(startIndex + i)
                      last_index = startIndex + i
      return result
...
if __name__=="__main__":
    s1 = """alksjdlfalksdlf"""
    arr1 = s1.split("\n")
    s2 = """alksdlflaksdjfsdf"""
    arr2 = s2.split("\n")
    diffs = diff_obj.diff_main(s1, s2)
    lcs = diff_obj.get_lcs_indexs_by_text1(diffs,s1)
    print lcs